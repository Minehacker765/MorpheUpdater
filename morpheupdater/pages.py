...
def build_showcase(cfg: dict, state: dict) -> bool:
    ...
    for key in sorted(state.get("builds", {})):
        ...
        cards_html += CARD.format(
            icon=icon, name=b.get("app_name") or pkg.rsplit(".", 1)[-1].capitalize(),
            pkg=pkg, ver=ver, arch=arch, patches=patches_str, dl=dl
        )

    if not cards_html:
        for app in cfg.get("apps", []):
            pkg = app["package"]
            cards_html += CARD.format(icon=f"icons/{pkg}.png", name=pkg.rsplit(".", 1)[-1].capitalize(),
                                      pkg=pkg, ver="—", arch=",".join(cfg.get("archs", [])), patches="—", dl="#")

    html = TEMPLATE.format(
        title=title, description=desc, repo_url=repo_url, fingerprint=fp or "—",
        updated=time.strftime("%Y-%m-%d %H:%M UTC", time.gmtime()),
        cards=cards_html, patches=patches,
    )
    dest = OUT / "index.html"
